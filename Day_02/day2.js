...
function solve(fileName) {
    ...
    return __awaiter(this, void 0, void 0, function () {
        var limit, open, file, ret, _d, _e, _f, line, gameId, games, validGame, e_1_1;
        return __generator(this, function (_g) {
            switch (_g.label) {
                ...
                case 4:
                    ...
                    _d = false;
                    line = _c;
                    gameId = parseInt(line.slice(5, line.indexOf(':')));
                    games = line.slice(line.indexOf(":") + 2).split(';');
                    validGame = games.map(function (game) {
                        var colors = game.trim().split(',');
                        return colors.map(function (color) {
                            var result = color.trim().split(' ');
                            console.log(result);
                            return result[0] <= limit[result[1]];
                        }).reduce(function (res, curr) { return res && curr; });
                    }).reduce(function (res, curr) { return res && curr; });
                    if (validGame) {
                        ret += gameId;
                    }
